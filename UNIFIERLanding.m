...
% Input bounds
problem.inputs.ul = [dElevmin dFlapmin DEPmin HTUmin];
problem.inputs.uu = [dElevmax dFlapmax DEPmax HTUmax];

% Initial input bounds
problem.inputs.u0l = [dElev0 dFlap0 DEP0 HTU0];
problem.inputs.u0u = [dElev0 dFlap0 DEP0 HTU0];

% Input rate bounds
problem.inputs.url = [-DUmax(3) -DUmax(4) -DUmax(5) -DUmax(7)];
problem.inputs.uru = [ DUmax(3)  DUmax(4)  DUmax(5)  DUmax(7)];

% Input constraint error bounds
problem.inputs.uConstraintTol  = [deg2rad(0.5) deg2rad(0.5) 0.1 0.1];
problem.inputs.urConstraintTol = [deg2rad(0.5) deg2rad(0.5) 0.1 0.1];

% Guess the input sequences with [u0 uf]
guess.inputs(:,1) = [dElev0 dElev0];
guess.inputs(:,2) = [dFlap0 dFlap0];
guess.inputs(:,3) = [DEP0   DEP0];
guess.inputs(:,4) = [HTU0   HTU0];
