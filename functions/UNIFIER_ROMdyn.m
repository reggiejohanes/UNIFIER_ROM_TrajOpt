...
if size(X,2)==1 % single vector input
    % States
    x     = X(1); % m
    z     = X(2); % m
    u     = X(3); % m/s
    ...
    % dFlap = U(:,4); % rad
    % dFlap = deg2rad(0);
    dFlap = linspace(runconfig.dFlap,runconfig.dFlap,numel(x))';
end
...
elseif runconfig.ROMfile==3 % v0 ROM
    if runconfig.ROMdep==1
        % Coefficients (all dependencies) ---------------------------------
